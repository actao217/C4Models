...
def train_networkA(num_games=150000, batch_size=100):
    netB = NetworkA(learning_rate=1e-3)
    model_id = 'X'
    opponent_id = 'O'
    ...
    plt.plot(x_vals, win_rates, label="ModelA Win Rate", linewidth=2)
    ...
    plt.title("NetworkA vs MCTS150")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("win_rates_networkA.png")
    plt.show()
